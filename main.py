...
def _ensure_state():
    ss = st.session_state
    ss.setdefault("users", {})
    ss.setdefault("teams", {})            # team_name -> {"captain":..., "members": set(), "city": "", "company": ""}
    ...
    ss.setdefault("user_challenges", {})  # per user progress
    ss.setdefault("team_battles", [])     # {team_a, team_b, start_date, end_date, reward_points, winner}
    ...
    ss.setdefault("collectibles", {})     # user_id -> {"coins": int, "items": set()}
    # Timer state
    ...
    if ch["id"] == "daily_5000":
        ...
        if tname and tname in st.session_state.teams:
            ...
            for member in members:
                ...
                if miles < float(ch["target_miles"]): ok_for_all = False; break
            if ok_for_all: uc["completed"] = True; add_points(uid, ch["reward_points"], ch["name"]); return True
...
def show_progress_ui(uid, ch):
    ...
    if uc["joined"] and not uc["completed"]:
        if st.button(f"Check & Complete '{ch['name']}'", key=f"complete_{ch['id']}"):
            completed = complete_challenge_if_eligible(uid, ch); st.success("✅ Completed!") if completed else st.warning("Not eligible yet—keep going!")
...
def settle_battles():
    ...
    for b in st.session_state.team_battles:
        if b["winner"] is None and today_iso > b["end_date"]:
            a_total = compute_team_distance_for_range(b["team_a"], b["start_date"], b["end_date"])
            b_total = compute_team_distance_for_range(b["team_b"], b["start_date"], b["end_date"])
            if a_total > b_total: b["winner"] = b["team_a"]
            elif b_total > a_total: b["winner"] = b["team_b"]
            else: b["winner"] = "draw"
    ...
    if st.session_state.get("timer_prompt_open"):
        ...
        with b1:
            if st.button("Save Walk"):
                ...
                update_challenges_after_walk(user_id)
...
with tab_community:
    ...
    if existing_battles:
        for b in existing_battles:
            ...
            if b["winner"]: st.success(f"Winner: {b['winner']} (+{b['reward_points']} pts per member)")
...
st.caption("Offline demo with in-memory storage. For production, add persistence (DB) and auth; connect Apple Health/Google Fit for automatic steps.")
